...
def vec_add(a: torch.Tensor, b: torch.Tensor, use_gather: bool = False,
    ...
    Args:
        a (torch.Tensor): The first input tensor. Must be 1D or 2D and on the CPU.
        ...
        use_gather (bool): If True, uses kernels with explicit gather/scatter and masking
                ...
                num_cpu_threads (int): Number of CPU threads used to run each kernel.
    ...
    Raises:
        ValueError: If input tensors have mismatched shapes, incorrect dimensions,
                    are not on the CPU, or have different data types.
    """
    # --- Input Validation ---
    if a.shape != b.shape:
    ...
    if a.dim() == 1:
        ...
        with cpu.compile_options({"num_cpu_threads": num_cpu_threads}):
            ct.launch(None, grid, kernel, (a, b, c, TILE))
    else:  # a.dim() == 2 (Matrix)
        ...
        with cpu.compile_options({"num_cpu_threads": num_cpu_threads}):
            ct.launch(None, grid, kernel, (a, b, c, TILE_X, TILE_Y))
...
if __name__ == "__main__":
    ...
    # Create random input tensors on the CPU.
    a_1d_direct = torch.randn(VECTOR_SIZE_1D, dtype=torch.float32, device='cpu')
    b_1d_direct = torch.randn(VECTOR_SIZE_1D, dtype=torch.float32, device='cpu')
    ...
    c_1d_cutile_direct = vec_add(
        a_1d_direct, b_1d_direct, use_gather=False,
    ...
    a_1d_gather = torch.randn(VECTOR_SIZE_1D_GATHER, dtype=torch.float32, device='cpu')
    b_1d_gather = torch.randn(VECTOR_SIZE_1D_GATHER, dtype=torch.float32, device='cpu')
    ...
    c_1d_cutile_gather = vec_add(
        a_1d_gather, b_1d_gather, use_gather=True,
        num_cpu_threads=args.num_cpu_threads)
    print(
    ...
    a_2d_direct = torch.randn(MATRIX_SHAPE_2D, dtype=torch.float32, device='cpu')
    b_2d_direct = torch.randn(MATRIX_SHAPE_2D, dtype=torch.float32, device='cpu')
    ...
    c_2d_cutile_direct = vec_add(
        a_2d_direct, b_2d_direct, use_gather=False,
        num_cpu_threads=args.num_cpu_threads)
    print(
    ...
    a_2d_gather = torch.randn(MATRIX_SHAPE_2D_GATHER, dtype=torch.float32, device='cpu')
    b_2d_gather = torch.randn(MATRIX_SHAPE_2D_GATHER, dtype=torch.float32, device='cpu')
    ...
    c_2d_cutile_gather = vec_add(
        a_2d_gather, b_2d_gather, use_gather=True,
        num_cpu_threads=args.num_cpu_threads)
    print(
