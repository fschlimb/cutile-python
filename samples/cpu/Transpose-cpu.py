...
def cutile_transpose(x: torch.Tensor, num_cpu_threads: int = 0) -> torch.Tensor:
    """
    ...
    Args:
        x (torch.Tensor): The input matrix (M x N).
                          This tensor *must* be 2D and on the CPU.

    Returns:
        torch.Tensor: The transposed matrix (N x M) on the CPU.

    Raises:
        ValueError: If the input tensor is not on the CPU or is not 2D.
    """
    # --- Input Validation ---
    if not x.is_cpu:
        raise ValueError("Input tensor must be on the CPU.")
    if x.ndim != 2:
    ...
    # The `transpose_kernel` is launched on the CPU with the calculated grid dimensions.
    ...
    with cpu.compile_options({"num_cpu_threads": num_cpu_threads}):
        ct.launch(None, grid, transpose_kernel, (x, y, tm, tn))
...
if __name__ == "__main__":
    ...
    # Create a random input matrix with float16 data type on the CPU.
    x_fp16 = torch.randn(M_dim, N_dim, dtype=torch.float16, device='cpu')
    print(f"Input x shape: {x_fp16.shape}, dtype: {x_fp16.dtype}")

    # Perform transposition using the cuTile wrapper function.
    y_fp16_cutile = cutile_transpose(x_fp16, args.num_cpu_threads)
    ...
    # Create a random input matrix with float32 data type on the CPU.
    x_fp32 = torch.randn(M_dim, N_dim, dtype=torch.float32, device='cpu')
    print(f"Input x shape: {x_fp32.shape}, dtype: {x_fp32.dtype}")

    # Perform transposition using the cuTile wrapper function.
    y_fp32_cutile = cutile_transpose(x_fp32, args.num_cpu_threads)
    ...
    M_dim_non_mult = 1000
    N_dim_non_mult = 500
    x_non_mult = torch.randn(M_dim_non_mult, N_dim_non_mult, dtype=torch.float32, device='cpu')
    print(f"Input x shape: {x_non_mult.shape}, dtype: {x_non_mult.dtype}")

    y_non_mult_cutile = cutile_transpose(x_non_mult, args.num_cpu_threads)
