...
def cutile_fmha(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor,
    ...
    if Q.device != K.device or Q.device != V.device or not Q.is_cpu:
        raise ValueError("All input tensors must be on the CPU.")
...
if __name__ == "__main__":
    ...
    output_fmha_cutile_non_causal = cutile_fmha(
        ...
        tile_m=32, tile_n=32,  # Increased tile sizes
        causal=False,
    ...
    if args.correctness_check:
        ...
        torch.testing.assert_close(output_fmha_cutile_non_causal, ref_fmha, atol=1e-2, rtol=1e-2)
