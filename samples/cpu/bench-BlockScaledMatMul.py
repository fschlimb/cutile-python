...
def block_quantize(x: torch.Tensor, block_size: int, dtype: torch.dtype = torch.float16):

    """
    Args:
        x (torch.Tensor):       input tensor.
        block_size (int):       size of block.
        dtype (torch.dtype):    datatype used to carry quantized values and scales on CPU.

    Returns:
        tuple[torch.Tensor, torch.Tensor, int]: A tuple containing:
            - x (torch.Tensor):         in dtype.
            - scale (torch.Tensor):     in dtype.
    ...
    scale = scale.to(dtype).squeeze(-1)
...
def unswizzle_32_4_4(tile_swizzled_scale):
    ...
    logical scale view used by the block-scaled matmul.
    '''
    m1, m2, k1 = 32, 4, 4
...
def block_scaled_matmul_kernel(
    ...
    for k in range(num_tiles_k):
        ...
        if len(A_scale.shape) == 2:
            ...
            # the logical block-scaled matmul shape (tm, tks).
        ...
        if len(B_scale.shape) == 2:
            ...
            # (tn, tks), then transpose it to the logical block-scaled shape (tks, tn).
...
def cutile_block_scaled_matmul(A: torch.Tensor, A_scale: torch.Tensor,
    ...
    Args:
        A (torch.Tensor):       The first input matrix (M x K). Must be on the CPU.
        B (torch.Tensor):       The second input matrix (K x N). Must be on the CPU
    ...
    Returns:
        torch.Tensor: The resulting matrix C (M x N) on the CPU.

    Raises:
        ValueError: If matrices are incompatible (K dimensions don't match),
                    or if they are not on the CPU.
    """
    # --- Input Validation ---
    ...
    if not A.is_cpu or not A_scale.is_cpu or not B.is_cpu or not B_scale.is_cpu:
    ...
    tm, tn, tk, scaling_block_size = 32, 32, 32, 16
    ...
    # The grid defines how many blocks will be launched.
    ...
    with cpu.compile_options({"num_cpu_threads": num_cpu_threads}):
        ct.launch(None, grid, kernel, (
...
if __name__ == "__main__":
    ...
    A = torch.rand((M_dim, K_dim), device="cpu", dtype=torch.float32)
    B = torch.rand((N_dim, K_dim), device="cpu", dtype=torch.float32)

    A, A_scale = block_quantize(A, scaling_block_size, dtype=torch.bfloat16)
    B, B_scale = block_quantize(B, scaling_block_size, dtype=torch.bfloat16)
    ...
    grid = (ceil(M_dim / tm) * ceil(N_dim / tn), 1, 1)
    benchmark_output = torch.empty(
        (M_dim, N_dim), device=A.device, dtype=torch.float32)

    ref_A_scale = torch.repeat_interleave(
        A_scale, scaling_block_size, dim=1).to(torch.float32)
    ref_B_scale = torch.repeat_interleave(
        B_scale, scaling_block_size, dim=0).to(torch.float32)
    ref_A = A.to(torch.float32) * ref_A_scale
    ref_B = B.to(torch.float32) * ref_B_scale
    stats_torch = report_benchmark(torch.matmul, (ref_A, ref_B))

    output_logical = cutile_block_scaled_matmul(
        A, A_scale, B, B_scale, args.num_cpu_threads)
    output_swizzled = cutile_block_scaled_matmul(
        A, A_s_swizzled, B, B_s_swizzled, args.num_cpu_threads)
    reference = ref_A @ ref_B
    torch.testing.assert_close(
        output_logical, reference, atol=1e-4, rtol=1e-3)
    torch.testing.assert_close(
        output_swizzled, reference, atol=1e-4, rtol=1e-3)
    print("Correctness check passed")

    benchmarks = (
        ("logical scales", A_scale, B_scale),
        ("swizzled scales", A_s_swizzled, B_s_swizzled),
    )
    print("Benchmark results:")
    for name, benchmark_A_scale, benchmark_B_scale in benchmarks:
        stats_cutile = report_benchmark(
            cutile_block_scaled_matmul,
            (A, benchmark_A_scale, B, benchmark_B_scale,
             args.num_cpu_threads),
            kernel=block_scaled_matmul_kernel,
            grid=grid,
            kernel_args=(A, benchmark_A_scale, B, benchmark_B_scale,
                         benchmark_output, tm, tn, tk, scaling_block_size),
