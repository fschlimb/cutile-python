...
# from cuda.tile.tune import exhaustive_search  # XPU: autotuning path disabled below
import torch
import math

from torch.nn.functional import scaled_dot_product_attention
# XPU: the CUDA-specific sdpa backends and the CUDA benchmark helper are unused here.
...
def cutile_fmha(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor,
    ...
    if Q.device != K.device or Q.device != V.device or not Q.is_xpu:
        raise ValueError("All input tensors must be on the same XPU device.")
    ...
    with xpu.compile_options(launch_options):
        ct.launch(torch.xpu.current_stream(), grid, kernel, (
            Q, K, V, Out,
            qk_scale,
            input_pos,
            D_k,
...
# XPU: autotuning relies on CUDA-specific launch hints (num_ctas/occupancy) and
# cuda.tile.tune.exhaustive_search; kept here (commented) for parity with the
...
def torch_fmha(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor,
    ...
    # XPU: the CUDA-specific sdpa backends (CUDNN/FLASH) are unavailable on XPU,
    # so we fall back to PyTorch's default scaled_dot_product_attention.
    # Original CUDA sample selected an explicit backend:
...
if __name__ == "__main__":
    ...
    print("--- Running cuTile Fused Multi-Head Attention (FMHA) Sample (XPU) ---")
    ...
    Q_input = torch.randn(BATCH_SIZE, NUM_HEADS, SEQ_LEN_Q, D_K, dtype=DTYPE, device=device)
    ...
    # XPU: kept (commented) for parity with the original CUDA sample. It depends on
    # tune_cutile_fmha (CUDA-specific launch hints), report_benchmark and CUDA sdpa
    # backends, none of which apply to the XPU backend.
    # print("\n--- Test 3: Causal Attention with autotuning and performance benchmarking ---")
