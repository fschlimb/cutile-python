...
@ct.kernel
def layer_norm_bwd_dx_partial_dwdb(DX, DY, DW, DB, X, W, Mean, Rstd, TILE_N: ConstInt):
    """
    Backward pass part 1: computes dX and partial dW/dB.
    Stores one partial gradient row per input row.
...
class CuTileLayerNorm(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input, weight, bias, eps, num_cpu_threads):
        """
        ...
        with cpu.compile_options({"num_cpu_threads": num_cpu_threads}):
            ct.launch(None, (M,), layer_norm_fwd,
                      (x, weight, bias, y, mean, rstd, eps, TILE_N))

        # Save tensors needed for the backward pass
        ctx.save_for_backward(x, weight, bias, mean, rstd)
        ctx.TILE_N = TILE_N
        ctx.num_cpu_threads = num_cpu_threads
    ...
    def backward(ctx, grad_output):
        ...
        TILE_N = ctx.TILE_N
        num_cpu_threads = ctx.num_cpu_threads
        M, N = x.shape
        ...
        # Allocate one partial gradient row per input row.
        ...
        with cpu.compile_options({"num_cpu_threads": num_cpu_threads}):
            ct.launch(None, (M,), layer_norm_bwd_dx_partial_dwdb,
        ...
        with cpu.compile_options({"num_cpu_threads": num_cpu_threads}):
            ct.launch(None, (math.ceil(N / TILE_N),), layer_norm_bwd_dwdb,
                      (dw, db, final_dw, final_db, TILE_M, TILE_N))

        return dx.reshape(*grad_output.shape), final_dw, final_db, None, None


def cutile_layer_norm(x, weight, bias, eps, num_cpu_threads=0):
    return CuTileLayerNorm.apply(x, weight, bias, eps, num_cpu_threads)
...
if __name__ == "__main__":
    ...
    shape = (1024, 2048)
    dtype = torch.float32
    weight = torch.randn(shape[-1], dtype=dtype, device='cpu', requires_grad=True)
    bias = torch.randn(shape[-1], dtype=dtype, device='cpu', requires_grad=True)
    x = -2.3 + 0.5 * torch.randn(shape, dtype=dtype, device='cpu')
    ...
    y = cutile_layer_norm(x, weight, bias, eps, args.num_cpu_threads)\
