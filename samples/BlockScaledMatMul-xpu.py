...
def block_quantize(x: torch.Tensor, block_size: int, dtype: torch.dtype = torch.float16):
    ...
    Args:
        ...
        dtype (torch.dtype):    the XPU-supported datatype used to store quantized values and
                                scales.

    Returns:
        tuple[torch.Tensor, torch.Tensor, int]: A tuple containing:
            - x (torch.Tensor):         normalized values in dtype.
            - scale (torch.Tensor):     power-of-two scale values in dtype.
    ...
    # The XPU backend host ABI does not accept FP8 tensors. Retain the FP8
    # normalization range while carrying values and power-of-two scales in FP16.
...
def unswizzle_32_4_4(tile_swizzled_scale):
    ...
    view used by the block-scaled matmul.
    '''
    m1, m2, k1 = 32, 4, 4
...
def block_scaled_matmul_kernel(
    ...
    The XPU lowering does not support ``ct.mma_scaled``, so scales are expanded along K
    and applied before the ordinary MMA.
    ...
    for k in range(num_tiles_k):
        ...
        if len(B_scale.shape) == 2:
            ...
            # (tn, tks), then transpose it to the logical block-scaled matmul shape (tks, tn).
        ...
        # Expand one scale factor per K block to the operand tiles, then use the XPU-supported
        # ordinary MMA operation instead of CUDA's scaled MMA instruction.
...
def cutile_block_scaled_matmul(A: torch.Tensor, A_scale: torch.Tensor,
    ...
    Args:
        A (torch.Tensor):       The first input matrix (M x K). Must be on an XPU device.
        B (torch.Tensor):       The second input matrix (K x N). Must be on an XPU device
    ...
    Returns:
        torch.Tensor: The resulting matrix C (M x N) on the XPU device.

    Raises:
        ValueError: If matrices are incompatible (K dimensions don't match),
                    do not meet XPU tile-alignment requirements, or are not on an XPU device.
    """
    # --- Input Validation ---
    ...
    if not A.is_xpu or not A_scale.is_xpu or not B.is_xpu or not B_scale.is_xpu:
    ...
    tm, tn, tk, scaling_block_size = 256, 256, 32, 32
    ...
    if m % tm != 0 or n % tn != 0 or k % tk != 0:
        raise ValueError(f"XPU block-scaled matmul requires M, N, and K to be multiples of "
                         f"{tm}, {tn}, and {tk}, respectively; got M={m}, N={n}, K={k}.")

    # --- Calculate Grid Dimensions for Kernel Launch (1D Grid) ---
    # The grid defines how many workgroups will be launched.
    # Each workgroup computes one (tm x tn) output tile of matrix C.
    grid_x = ct.cdiv(m, tm)  # Number of workgroups needed along the M dimension (rows of C)
    grid_y = ct.cdiv(n, tn)  # Number of workgroups needed along the N dimension (columns of C)
    ...
    # on the same device, and with float32 accumulation precision.
    ...
    with xpu.compile_options(options):
        ct.launch(torch.xpu.current_stream(), grid, block_scaled_matmul_kernel, (
...
if __name__ == "__main__":
    ...
    print("--- Running cuTile Block Scaled Matrix Multiplication Examples (XPU) ---")
    ...
    A = torch.rand((M_dim, K_dim), device='xpu')
    B = torch.rand((N_dim, K_dim), device='xpu')
