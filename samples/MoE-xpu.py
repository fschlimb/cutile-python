...
def cutile_moe(
    ...
    Args:
        ...
        options: XPU compile options forwarded through ``xpu.compile_options(options)``.
...
def invoke_silu_and_mul_kernel(
    ...
    TILE_N = next_power_of_2(C.shape[-1])

    # The silu kernel is elementwise over a (1, TILE_N) tile per block.
    launch_options = {
        "wg_m": 1,
        "wg_n": TILE_N,
        **(options or {}),
...
if __name__ == "__main__":
    ...
    print("--- Running cuTile Mixture-of-Experts (MoE) Sample (XPU) ---")
